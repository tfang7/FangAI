...
public class Base {
	...
	public Base(Unit u, Game g, BuildingUtil helper)
	{
		builder = helper;
		ebayBuilt = false;
		refineryBuilt = false;
	...
	{
		if (!workers.contains(u) && !builders.contains(u) && !gassers.contains(u) && scout.getID() != u.getID())
		{
			workers.add(u);
		}
	}
